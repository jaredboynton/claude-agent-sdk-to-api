...
export function buildCodeToolDescription(clientTools, { caveman = cavemanLevels().tools } = {}) {
  ...
  const head = (
    ...
    + "`await codemode.exec(source, { interpreter?, interpreterArgs?, cwd?, args?, tool? })` runs an inline script through the client's shell tool with zero quoting hazards: the source is base64-armored into a mktemp file and executed in one atomic command (interpreter defaults to `node`; `.mjs`/`.cjs` inferred from the source; `interpreterArgs` are CLI flags placed BEFORE the script path, e.g. `[\"--expose-internals\"]`, while `args` become the script's argv). The temp file lives in a random mktemp dir and is deleted when the command finishes — to re-run, call `exec` again; never reference the temp path from a later shell call. Always use it instead of `node -e`/`python -c` one-liners — the shell mangles `$[`, `$(`, and nested quotes inside those — and instead of separate write-temp-file-then-run steps. The client shell on macOS is BSD userland: GNU-only extensions (`sed -n 'N,+Kp'`, `grep -P`, `date -d`) fail there, so put anything beyond a trivial one-liner in `codemode.exec` instead of stacking sed/awk flags. When grepping for literal code, pass `-F` to `rg`/`grep` — code is full of regex metacharacters (`(`, `[`, `.`, `+`) that break pattern parsing; for an OR over several literals use repeated `-F -e 'literal'` flags, and write a real regex only when you actually need one. If a pattern fails to parse, fix it and retry in-script — never return just to report a bad pattern. "
